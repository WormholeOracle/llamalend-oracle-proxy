...
from brownie import Contract, accounts, chain, CryptoFromPoolsVaultWAgg, OracleProxy, AMM, return_0
...
def test_zero():
	...
	dummy_impl = return_0.deploy({'from': accounts[0]})
	...
	# set impl2
	proxy.set_price_oracle(dummy_impl.address, {'from': admin})
...
def test_borked():
	...
	# Deploy the Proxy contract, using the factory's address to get the admin
	proxy = OracleProxy.deploy(
		dummy_impl.address, # implementation
		factory.address, # factory
		500, # Max deviation in BPS
		{'from': accounts[0]}
	)
	...
	# set impl2
	proxy.set_price_oracle(oracle_impl.address, {'from': admin})
